...
wmata <- wmata %>%
  ...
  filter(!substr(Date, 3, 4) == "04" & !substr(Date, 3, 4) == "05")

monthly <- wmata %>%
  mutate(month = factor(substr(Date, 6, 7))) %>%
  group_split(month)
...
```{r}
monthlyboxplots <- monthly %>%
  map(~ {unique(.x$month)
  
  ggplot(data = .x, aes(x = prop, y = month, fill = month)) +
  geom_boxplot() +
...
```{r}
keep(c(2024, 3021, 2000, 1800, 800, 5050, 600, 7156), leap_year)
```
