...
```{r}
st_merged <- left_join(st$starship, st$captain, by = c("ID" = "ship"))
...
```{r}
price_depthmodel <- lm(price ~ depth, data = diamonds)
summary(price_depthmodel)

summary(price_depthmodel)$coefficients["depth", "Pr(>|t|)"]
```
...
ggplot(data = diamonds, mapping = aes(x = depth, y = price)) +
  ...
  theme_minimal()
```
No, the plots do not match what was indicated by the small p-value. The p-value 
indicated that depth was a much stronger predictor of diamond price since it was 
smaller than the alpha level 0.05. However,based on the graphs faceted by diamond 
color, there is not a strong relationship between the depth and price of diamonds. 
The regression line is nearly flat or indicates a slight negative relationship 
in most of these graphs. 
